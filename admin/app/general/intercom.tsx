...
export const IntercomConfig = observer(({ isTelemetryEnabled }: TIntercomConfig) => {
  ...
  const submitInstanceConfiguration = async (payload: Partial<IFormattedInstanceConfiguration>) => {
    try {
  ...
  const enableIntercomConfig = () => {
    submitInstanceConfiguration({ IS_INTERCOM_ENABLED: isIntercomEnabled ? "0" : "1" });
  };
    ...
    <div className="flex items-center gap-14 px-4 py-3 border border-custom-border-200 rounded">
      <div className="grow flex items-center gap-4">
        ...
        <div className="ml-auto">
          <ToggleSwitch
            value={Boolean(isIntercomEnabled)}
            onChange={enableIntercomConfig}
            size="sm"
            disabled={!isIntercomEnabled || isSubmitting || initialLoader}
          />
        </div>
      </div>
    </div>
