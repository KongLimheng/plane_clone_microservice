...
export const HelpSection = observer(() => {
  ...
  return (
    ...
    >
      <div className={cn("flex items-center gap-1", isSidebarCollapsed ? "flex-col justify-center" : "w-full")}>
        <Tooltip tooltipContent="Redirect to plane" position="right" className="ml-4" disabled={!isSidebarCollapsed}>
          <a
            href={redirectionLink}
            className={cn(
              "relative px-2 py-1.5 flex items-center gap-2 font-medium rounded border border-custom-primary-100/20 bg-custom-primary-100/10 text-xs text-custom-primary-200 whitespace-nowrap"
            )}
          >
      ...
      <div className="relative">
        ...
        >
          <div
            ref={helpOptionsRef}
            className={cn(
              "absolute bottom-2 min-w-[10rem] z-[15]",
              isSidebarCollapsed ? "left-full" : "-left-[75px]",
              "divide-y divide-custom-border-200 whitespace-nowrap rounded bg-custom-background-100 p-1 shadow-custom-shadow-xs"
            )}
          >
            <div className="space-y-1 pb-2">
              {helpOptions.map(({ href, Icon, name }) =>
                !href ? (
