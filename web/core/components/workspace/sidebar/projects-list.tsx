...
import { useAppTheme, useCommandPalette, useRouterParams, useUserPermissions } from "@/hooks/store";
...
export const SidebarProjectsList = observer(() => {
  ...
  const handleOnProjectDrop = (
    ...
    joinedProjects.map((pid) => {
      const projectDetails = getProjectById(pid);
  ...
  return (
    <>
      ...
      <div ref={containerRef} className={cn({ "border-t border-custom-sidebar-border-300": isScrolled })}>
        <>
          <Disclosure as={"div"} className={"flex flex-col"} defaultOpen={isAllProjectsListOpen}>
            ...
            >
              ...
              {!isCollapsed && (
                <div className="flex items-center opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto">
                  {isAuthorizedUser && (
                    <Tooltip tooltipContent="" tooltipHeading="Create Project">
                      <button
                        onClick={() => {
                  ...
                  >
                    <ChevronRight
                      className={cn("flex-shrink-0 size-4 transition-all", { "rotate-90": isAllProjectsListOpen })}
                    />
                  </Disclosure.Button>
                </div>
              )}
            ...
            >
              {isAllProjectsListOpen && (
                <Disclosure.Panel as="div" className={cn("space-y-1", { "space-y-0 ml-0": isCollapsed })} static>
                  {joinedProjects.map((projectId, idx) => (
                    <SidebarProjectsListItem
                      ...
                      disableDrag={false}
                      disableDrop={false}
                      isLastChild={idx === joinedProjects.length - 1}
        ...
        {isAuthorizedUser && joinedProjects?.length === 0 && (
          <button
            ...
            className={cn(
              ...
              { "p-0 size-8 aspect-square justify-center mx-auto": isCollapsed }
            )}
            onClick={() => {
