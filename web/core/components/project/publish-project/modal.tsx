...
import { Button, CustomSelect, EModalWidth, Loader, ModalCore, setToast, TOAST_TYPE, ToggleSwitch } from "@plane/ui";
import { SPACE_BASE_PATH, SPACE_BASE_URL } from "@/helpers/common.helper";
import { copyTextToClipboard } from "@/helpers/string.helper";
import { useProjectPublish, useRouterParams } from "@/hooks/store";
...
export const PublishProjectModal = observer(({ isOpen, project, onClose }: Props) => {
  ...
  useEffect(() => {
    ...
    if (!projectPublishSettings) {
      fetchPublishSettings(workspaceSlug, project.id);
    }
  ...
  const handlePublishProject = async (payload: Partial<TProjectPublishSettings>) => {
    ...
    await publishProject(workspaceSlug, project.id, payload);
  };

  const handleUpdatePublishSettings = async (payload: Partial<TProjectPublishSettings>) => {
    if (!workspaceSlug || !payload.id) return;

    await updatePublishSettings(workspaceSlug, project.id, payload.id, payload).then((res) => {
      setToast({
    ...
    <ModalCore isOpen={isOpen} handleClose={handleClose} width={EModalWidth.XXL}>
      <form onSubmit={handleSubmit(handleFormSubmit)}>
        <div className="flex items-center justify-between gap-2 p-5">
          ...
          {project.anchor && (
            <Button variant="danger" loading={isUnPublishing} onClick={() => handleUnPublishProject(watch("id") ?? "")}>
        ...
        {fetchSettingsLoader ? (
          ...
          <div className="px-5 space-y-4">
            {project.anchor && projectPublishSettings && (
              <>
                <div className="bg-custom-background-80 border border-custom-border-300 rounded-md py-1.5 pr-1 pl-4 flex items-center justify-between gap-2">
                  <a
                    href={publishLink}
                    target="_blank"
        ...
        {/* modal handlers */}
        <div className="relative flex items-center justify-between border-t border-custom-border-200 px-2 py-4 mt-4">
