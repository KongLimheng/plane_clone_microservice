...
export const ComicBoxButton = ({ label, icon, title, description, onClick, disabled = false }: Props) => {
    ...
    <Popover as="div" className={"relative"}>
      <Popover.Button as={Fragment}>
        <button type="button" ref={setReferenceElement} onClick={onClick} disabled={disabled}>
          <div className={cn("flex items-center gap-2.5", getButtonStyling("primary", "lg", disabled))}>
            {icon}
      ...
      {isHovered && (
        <Popover.Panel static className={"fixed z-10"}>
          <div
