...
export const OverviewStatsWidget: FC<WidgetProps> = observer((props) => {
  ...
  if (isLoading) return <WidgetLoader widgetKey={WIDGET_KEY} />;
  ...
  return (
    ...
    >
      {STATS_LIST.map((stat, idx) => (
        <div
          key={stat.key}
          className={cn("w-full flex flex-col gap-2 hover:bg-custom-background-80", {
            "rounded-l-md": idx === 0,
            "rounded-r-md": idx === STATS_LIST.length - 1,
            "rounded-tr-xl lg:rounded-[0px]": idx === 1,
            "rounded-bl-xl lg:rounded-[0px]": idx === 2,
          })}
        >
          <Link href={stat.link} className="py-4 duration-300 rounded-[10px] w-full">
            <div className={cn("relative flex pl-10 sm:pl-20 md:pl-20 items-center")}>
              <div>
