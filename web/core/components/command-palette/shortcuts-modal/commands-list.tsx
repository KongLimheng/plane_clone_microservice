...
export const ShortcutCommandsList = ({ searchQuery }: Props) => {
  ...
  const filteredShortcuts = KEYBOARD_SHORTCUTS.map((cat) => {
  ...
  return (
    <div className="flex flex-col gap-y-3 overflow-y-auto">
      {!isShortcutsEmpty ? (
        filteredShortcuts.map(({ key, title, shortcuts }) => (
          <div key={key}>
            <h5 className="text-left text-sm font-medium">{title}</h5>
