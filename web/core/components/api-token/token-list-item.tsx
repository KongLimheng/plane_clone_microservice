...
import { DeleteApiTokenModal } from "./delete-token-modal";

type Props = {
  token: IApiToken;
};

export const ApiTokenListItem = ({ token }: Props) => {
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  // hooks
  const { isMobile } = usePlatformOS();
  return (
    <>
      <DeleteApiTokenModal tokenId={token.id} isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} />
