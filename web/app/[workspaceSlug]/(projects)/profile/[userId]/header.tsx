...
import { Breadcrumbs, CustomMenu, Header, UserActivityIcon } from "@plane/ui";
...
import { useAppTheme, useRouterParams, useUser, useUserPermissions } from "@/hooks/store";
...
export const UserProfileHeader = observer(
  ({ userProjectsData, type = undefined, showProfileIssuesFilter }: TUserProfileHeader) => {
    // router
      ...
      <Header>
            ...
            <Breadcrumbs.BreadcrumbItem
              ...
              link={
                <BreadcrumbLink
                  ...
                  disableTooltip
                  icon={<UserActivityIcon className="size-4 text-custom-text-300" />}
                />
              }
            />
          </Breadcrumbs>
        ...
        <Header.RightItem>
          ...
          <div className="flex gap-4 md:hidden">
            ...
            >
              {tabsList.map((tab) => (
                <CustomMenu.MenuItem key={tab.route} className="flex items-center gap-2">
                  <Link
                    key={tab.route}
            ...
            <button type="button" className="block transition-all md:hidden" onClick={() => toggleProfileSidebar()}>
              <PanelRight
                className={cn(
