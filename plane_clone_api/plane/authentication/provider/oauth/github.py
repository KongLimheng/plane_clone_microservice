...
import requests
from plane.authentication.adapter import OauthAdapter
from plane.license.utils.instance_value import get_configuration_value
from plane.settings.common import env
from plane.authentication.adapter.error import GITHUB_NOT_CONFIGURED, GITHUB_OAUTH_PROVIDER_ERROR, AuthenticationException
...
class GitHubOAuthProvider(OauthAdapter):
    ...
    def __init__(self, request,  code=None, state=None, callback=None):
        GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET = get_configuration_value(
            [
                ...
                },
                {
                    ...
                    "default": env("GITHUB_CLIENT_SECRET"),
                },
            ]
        )
        ...
        super().__init__(request, self.provider, client_id, self.scope, redirect_uri,
                         auth_url, self.token_url, self.userinfo_url, client_secret, code, callback)
    ...
    def __get_email(self, headers):
        try:
            ...
            email = next((email['email']
                         for email in emails_response if email["primary"]), None)
            return email
        except requests.RequestException:
            raise AuthenticationException(
                error_code=GITHUB_OAUTH_PROVIDER_ERROR,
