...
from plane.authentication.adapter.error import ADMIN_ALREADY_EXIST, ADMIN_AUTHENTICATION_FAILED, ADMIN_USER_ALREADY_EXIST, ADMIN_USER_DEACTIVATED, ADMIN_USER_DOES_NOT_EXIST, INVALID_ADMIN_EMAIL, INVALID_ADMIN_PASSWORD, REQUIRED_ADMIN_EMAIL_PASSWORD, REQUIRED_ADMIN_EMAIL_PASSWORD_FIRST_NAME, AuthenticationException, INSTANCE_NOT_CONFIGURED
from plane.authentication.utils.host import base_host, user_ip
from plane.authentication.utils.login import user_login
from plane.db.models import User, Profile
from plane.license.api.serializers import InstanceAdminMeSerializer, InstanceAdminSerializer
...
class InstanceAdminSignUpEndpoint(View):
    ...
    def post(self, request: HttpRequest):
        ...
        if not email or not password or not first_name:
            exc = AuthenticationException(
                error_code=REQUIRED_ADMIN_EMAIL_PASSWORD_FIRST_NAME,
        ...
        try:
            ...
            exc = AuthenticationException(
                error_code=INVALID_ADMIN_EMAIL,
        ...
        if User.objects.filter(email=email).exists():
            exc = AuthenticationException(
                error_code=ADMIN_USER_ALREADY_EXIST,
...
class InstanceAdminSignInEndpoint(View):
    ...
    def post(self, request: HttpRequest):
        ...
        if not email or not password:
            exc = AuthenticationException(
                error_code=REQUIRED_ADMIN_EMAIL_PASSWORD,
        ...
        if not user:
            exc = AuthenticationException(
                error_code=ADMIN_USER_DOES_NOT_EXIST,
