    ...
    >
      ...
      {isOpen && (
        <Listbox.Options className={"fixed z-20"} onClick={closeDropdown} static>
          <div
            className={cn(
              "my-1 overflow-y-scroll vertical-scrollbar scrollbar-sm rounded-md border-[0.5px] border-custom-border-300 bg-custom-background-100 px-2 py-2.5 text-xs shadow-custom-shadow-rg focus:outline-none min-w-[12rem] whitespace-nowrap",
              {
...
const Option = (props: ICustomSelectItemProps) => {
  ...
  return (
    <Listbox.Option
      ...
      className={({ active }) =>
        cn(
          "cursor-pointer select-none truncate rounded px-1 py-1.5 text-custom-text-200 items-center flex justify-between gap-2",
          {
